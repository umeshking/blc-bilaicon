...
{
  _init_complete.wait();
  QSettings("bilaicoin", BTS_BLOCKCHAIN_NAME).setValue("crash_state", "no_crash");
  if (_client)
     _bilaicoin_thread.async([this]{
       _client->stop();
       /*
...
void ClientWrapper::initialize(INotifier* notifier)
{
  ...
  _init_complete = _bilaicoin_thread.async( [=](){
    try
    {
...
{
  fc::variant_object result = _bilaicoin_thread.async( [this](){ return _client->get_info(); }).wait();
