...
BilaicoinApp* BilaicoinApp::_instance = nullptr;

#define APP_NAME "Bilaicoin"

static std::string CreateBilaicoinVersionNumberString()
{
   return bts::client::version_info()["client_version"].as_string();
}

BilaicoinApp::BilaicoinApp(int& argc, char** argv)
...
BilaicoinApp::~BilaicoinApp()
{
   assert(_instance == this && "Only one instance allowed at time");
   _instance = nullptr;
}

int BilaicoinApp::run(int& argc, char** argv)
{
   installCrashRptHandler(APP_NAME, CreateBilaicoinVersionNumberString().c_str());

   BilaicoinApp app(argc, argv);
   QTranslator bilaicoinTranslator;
   bilaicoinTranslator.load(QLocale::system().name()+".qm");
   app.installTranslator(&bilaicoinTranslator);
...
int BilaicoinApp::run()
{
   setOrganizationName("Bilaicoin");
   setOrganizationDomain("bilaicoin.org");
...
void BilaicoinApp::prepareStartupSequence(ClientWrapper* client, Html5Viewer* viewer, MainWindow* mainWindow, QSplashScreen* splash)
{
   viewer->connect(viewer->webView(), &QGraphicsWebView::urlChanged, [viewer,client,mainWindow] (const QUrl& newUrl) {
      ...
      viewer->webView()->page()->mainFrame()->addToJavaScriptWindowObject("bilaicoin", client);
...
QLocalServer* BilaicoinApp::startSingleInstanceServer(MainWindow* mainWindow)
{
...
bool BilaicoinApp::notify(QObject* receiver, QEvent* e)
{
   APP_TRY
   {
...
void BilaicoinApp::onExceptionCaught(const fc::exception& e)
{
   displayFailureInfo(e.to_detail_string());
}

void BilaicoinApp::onUnknownExceptionCaught()
{
   std::string detail("Unknown exception caught");
   displayFailureInfo(detail);
}

void BilaicoinApp::displayFailureInfo(const std::string& detail)
{
   elog("${e}", ("e", detail));
