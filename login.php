<//-- This is a proof of concept web application for Bilaicoin logins. -->
<?PHP
    //CONFIG
    $RPC_SERVER_USER = "a";
    $RPC_SERVER_PASS = "a";
...
//    $LOGIN_PAGE_DOMAIN_AND_PATH = "Set this if login landing page != this page";
    ...
    $bilaicoin = new Bitcoin($RPC_SERVER_USER, $RPC_SERVER_PASS, $RPC_SERVER_ADDRESS, $RPC_SERVER_PORT, $RPC_SERVER_PATH);

    $bilaicoin->open($RPC_SERVER_WALLET);
    $bilaicoin->unlock(2, $RPC_SERVER_WALLET_PASS);   
    
    if(isset($_REQUEST["client_key"]))
    {
        $loginPackage = $bilaicoin->wallet_login_finish($_REQUEST["server_key"], $_REQUEST["client_key"], $_REQUEST["signed_secret"]);
        ...
        $userAccount = $bilaicoin->blockchain_get_account($loginPackage["user_account_key"]);
        ...
        $url = $bilaicoin->wallet_login_start($BITSHARES_USER_NAME);
        $url .= $absolute_url;
        ?>
bilaicoin will now prompt you to complete the login in a new window. You may now close this window.
